...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusscihuidanci');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','discusscihuidanci');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('discusscihuidanci', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussdiscusscihuidanci', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscusscihuidanci/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discusscihuidanci', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discusscihuidanci', this.detail);
				this.$utils.msg('审核成功');
