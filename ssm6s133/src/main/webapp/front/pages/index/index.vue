<template>
	<view class="uni-padding-wrap">
		<view class="header">
			...
			<view class="right-content" @tap="onPageTap('cihuidanci')" :style="{fontSize:'32rpx',color:'rgba(255, 255, 255, 1)'}">
				查看更多
			</view>
		</view>
		<view v-if='1 == 1 || 1 == 2' class="uni-product-list" :class="1 == 2 ? 'active' : ''" :style='{"borderRadius":0,"backgroundColor":"rgba(135, 128, 128, 0.34)"}'>
			<view @tap="onDetailTap('cihuidanci',product.id)" :style='{"borderRadius":"12rpx","backgroundColor":"#fff"}' class="uni-product" v-for="(product,index) in homecihuidancilist" :key="index">
				                				<view :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"center"}' class="uni-product-title">{{product.danci}}</view>
								                				                				                				                				<view class="image-view" :style='{"borderRadius":"20rpx","height":"320rpx"}'>
					<image :style='{"borderRadius":"20rpx","height":"320rpx"}' mode="aspectFill" class="uni-product-image" :src="product.fengmian?product.fengmian.split(',')[0]:''"></image>
				</view>
                							</view>
		</view>
		<view class="list" v-if='1 == 3' :style='{"borderRadius":0,"backgroundColor":"rgba(135, 128, 128, 0.34)"}'>
			<view :style='{"borderRadius":"12rpx","backgroundColor":"#fff"}' @tap="onDetailTap('cihuidanci',product.id)" v-for="(product,index) in homecihuidancilist" :key="index" class="listm flex flex-between">
																<view class="listmr">
					<view class="col3 f30 elip mb15" :style='{"fontSize":"28rpx","lineHeight":"56rpx","color":"#333","textAlign":"center"}'>{{product.danci}}</view>
				</view>
																																																																																				<image :style='{"borderRadius":"20rpx","height":"320rpx"}' mode="aspectFill" class="listmpic" :src="product.fengmian?product.fengmian.split(',')[0]:''"></image>
																							</view>
		</view>
		<!--list end-->
	...
	export default {
		...
		async onShow() {
			...
			this.news = res.data.list
															...
															res = await this.$api.list('cihuidanci', {
				page: 1,
				limit: 4
			});
			this.homecihuidancilist = res.data.list
																																																		},
		methods: {
			//轮播图跳转
			onSwiperTap(e) {
