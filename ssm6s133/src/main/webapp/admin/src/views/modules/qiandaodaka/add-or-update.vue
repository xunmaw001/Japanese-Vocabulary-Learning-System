        ...
        <div v-else>
          <el-form-item v-if="ruleForm.touxiang" label="头像" prop="touxiang">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.touxiang.split(',')" :src="item" width="100" height="100">
                                    ...
                                    <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="计划词量" prop="jihuaciliang">
          <el-input v-model="ruleForm.jihuaciliang" 
              placeholder="计划词量" clearable  :readonly="ro.jihuaciliang"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="计划词量" prop="jihuaciliang">
              <el-input v-model="ruleForm.jihuaciliang" 
                placeholder="计划词量" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
                                    <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `qiandaodaka/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.qiandaodakaCrossAddOrUpdateFlag = false;
                  this.parent.search();
